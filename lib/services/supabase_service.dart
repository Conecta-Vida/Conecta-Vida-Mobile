...
class SupabaseService {
  ...
  static Future<void> syncDefaultNoticias() async {
    try {
      ...
      developer.log('Noticias sincronizadas com sucesso: ${insertResponse.length} registros');
    } catch (e) {
      developer.log('Erro ao sincronizar noticias: $e', name: 'SupabaseService');
      rethrow;
    }
  }
  ...
  static Future<void> upsertUsuario(UserModel user, {String senha = ''}) async {
    try {
      ...
      developer.log('Usuario ${user.email} sincronizado com sucesso', name: 'SupabaseService');
    } catch (e) {
      developer.log('Erro ao sincronizar usuario ${user.email}: $e', name: 'SupabaseService');
      rethrow;
    }
  }
  ...
  static Future<UserModel?> fetchUsuarioByEmail(String email) async {
    try {
      ...
      if (response.data == null) {
        developer.log('Usuario com email $email nao encontrado', name: 'SupabaseService');
        return null;
      }
      ...
      if (e.code == 'PGRST116') {
        ...
        developer.log('Usuario com email $email nao existe', name: 'SupabaseService');
        return null;
      }
      developer.log('Erro ao buscar usuario $email: $e', name: 'SupabaseService');
      return null;
    } catch (e) {
      developer.log('Erro inesperado ao buscar usuario $email: $e', name: 'SupabaseService');
      return null;
    }
  }
  ...
  static Future<void> signUp(String email, String senha) async {
    try {
      ...
      developer.log('Usuario $email registrado com sucesso', name: 'SupabaseService');
    } on AuthException catch (e) {
      developer.log('Erro de autenticacao ao registrar: ${e.message}', name: 'SupabaseService');
      throw Exception('Falha no cadastro: ${e.message}');
    } catch (error) {
      developer.log('Erro ao registrar usuario: $error', name: 'SupabaseService');
  ...
  static Future<void> signIn(String email, String senha) async {
    try {
      ...
      developer.log('Usuario $email autenticado com sucesso', name: 'SupabaseService');
    } on AuthException catch (e) {
      developer.log('Erro de autenticacao ao fazer login: ${e.message}', name: 'SupabaseService');
  ...
  static Future<void> resetPassword(String email) async {
    try {
      ...
      developer.log('Email de recuperacao enviado para $email', name: 'SupabaseService');
    } on AuthException catch (e) {
      developer.log('Erro ao enviar email de recuperacao: ${e.message}', name: 'SupabaseService');
      throw Exception('Falha ao enviar email de recuperacao: ${e.message}');
    } catch (error) {
      developer.log('Erro ao enviar email de recuperacao: $error', name: 'SupabaseService');
  ...
  static Future<List<NewsModel>> fetchNoticiasPorCategoria(
    ...
    try {
      PostgrestFilterBuilder query =
          Supabase.instance.client.from('noticias').select();
      ...
      if (response.data == null || (response.data is List && (response.data as List).isEmpty)) {
        developer.log('Nenhuma noticia encontrada para categoria: $categoria',
      ...
      developer.log('Erro ao buscar noticias da categoria $categoria: $e',
          name: 'SupabaseService');
      return NewsController().getNoticiasPorCategoria(categoryIndex);
    }
  }

  static Future<void> trackNewsShare(NewsModel noticia,
      {String sharedBy = 'app_user'}) async {
    try {
      ...
      developer.log('Compartilhamento de noticia rastreado: ${noticia.titulo}',
          name: 'SupabaseService');
    } on PostgrestException catch (e) {
      // Tabela pode nao existir ainda
      developer.log('Erro ao rastrear compartilhamento: ${e.message}',
          name: 'SupabaseService');
    } catch (e) {
      developer.log('Erro ao rastrear compartilhamento: $e',
          name: 'SupabaseService');
    }
  }
